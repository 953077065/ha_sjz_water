...
- 全部参数可在「选项」中随时修改(鉴权信息、账期范围、轮询间隔)
...
| `verify` | 是 | — | 验证码 (verfiy) |
| `cookie` | 否 | — | Cookie 字符串 |
| `start_bm` | 否 | `202409` | 起始账期 (YYYYMM) |
| `end_bm` | 否 | `202609` | 结束账期 (YYYYMM) |
| `scan_interval` | 否 | `604800` | 轮询间隔秒数 (默认 1 周, 最小 10) |

添加后可在「集成 → 配置」中随时修改除 `base_url` 外的所有参数。
...
| 账期 | `billingMonth` | DATE | — |
| 抄表时间 | `readDate` | TIMESTAMP | — |
| 下次抄表时间 | `nextReadDate` | TIMESTAMP | — |
| 缴费状态 | `payState` | — | 未缴费/部分缴费/已缴费 |
| 缴费时间 | `payTime` | TIMESTAMP | — |
| 用户姓名 | `cardName` | — | — |
| 用户地址 | `cardAddress` | — | — |

## 历史趋势查看

集成首次加载时会自动回填所有历史月份的数据到 HA Recorder, 之后在 **历史面板** 搜索 `本期用水量` 或 `本期应缴金额` 即可看到月度趋势曲线。
